...
The repository is now `Cyletix/CyIME`; former `Cyletix/Xime` URLs redirect to it. Existing releases may still carry the former name. New builds use `CyIME-0.1.5-<abi>.apk`. The application ID and signing configuration remain compatible with the existing fork.
